...
public class EnemySpawner : MonoBehaviour
{
    ...
    public IEnumerator SpawnFromAPI()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (eop == null)
            {
                eop = enemy.AddComponent<EnemyOriginalPosition>();
            }
            eop.SetOriginalPosition(spawn.position);
            
            SpriteRenderer sr = enemy.GetComponent<SpriteRenderer>();
            if (sr != null)
            {
                eop.SetOriginalFlip(sr.flipX);
            }
            
            Debug.Log(enemy.name + " spawned at: " + spawn.position);
